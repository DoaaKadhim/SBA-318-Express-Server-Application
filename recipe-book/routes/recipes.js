...
router.post('/', validateRecipe, (req, res) => {
  // Find the maximum ID and increment by 1 for the new recipe
  const maxId = recipes.reduce((max, recipe) => Math.max(max, recipe.id), 0);
  const newRecipe = {
    id: maxId + 1, // Ensure new IDs are unique
    name: req.body.name,
    ingredients: req.body.ingredients,
    instructions: req.body.instructions
  };
  recipes.push(newRecipe); // Add new recipe to the end of the array
